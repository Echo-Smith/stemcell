...
def build():
    ...
    add_compact_bullet(doc, bullet_id, "收敛 MVP：", "在缺少真实行为数据的早期阶段，不做黑盒个性化推荐；以孕产阶段和 6 类状态入口降低表达门槛，将推荐从“给一首歌”定义为由行动、音乐 / 自然声和呼吸提示组成的可解释陪伴方案，并以 PWA 降低首次体验成本。", 8.0)
    add_compact_bullet(doc, bullet_id, "划定安全边界：", "将模型限定在状态理解与陪伴表达，设置 S0–S4 风险分级；高风险输入停止普通推荐并转为安全提醒，普通场景按状态标签与受控曲库匹配，同时解释推荐原因。", 8.0)
    add_compact_bullet(doc, bullet_id, "以高风险语义驱动评测：", "针对否定词 / 风险词漏识别可能反转语义的问题，以 177 条合成音频和 30 条产品用例对比 4 款 ASR、4 款 LLM，完成 700+ 次调用；MiMo ASR 在当前测试集 CER 0.40%，48 条否定词与 36 条风险词样本均完整保留。样本仍以合成音频为主，结果仅用于初步选型；项目从 200+ 份提案中入围 14 项决赛作品。", 8.0)
    ...
    add_compact_bullet(doc, bullet_id, "官号与矩阵运营（粉丝 2 万+）：", "负责今日头条、微博、小红书等官号的内容适配、发布排期与数据监测；参与约 150 人素人号矩阵的账号激活、任务分发、发布巡检和反馈回收。根据平台形态与传播节点调整选题呈现和更新节奏，2025 年今日头条账号粉丝达 2 万+、日均展现约 3 万，日均分发重点内容约 3 条。", 8.0)
    add_compact_bullet(doc, bullet_id, "专项传播协同：", "围绕重点节点和跨部门专项任务，将任务要求拆分为执行清单，组织多方协作并汇总过程数据；根据阶段复盘调整选题呈现、发布节奏和协作方式，支持相关工作表现保持市级前列。", 8.0)
